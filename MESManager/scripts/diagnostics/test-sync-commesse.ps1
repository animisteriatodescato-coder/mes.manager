...
sqlcmd -S "localhost\SQLEXPRESS01" -d MESManager -E -C -Q "UPDATE SyncStates SET UltimaSyncRiuscita = '2020-01-01' WHERE Modulo = 'Commesse'" -b
...
sqlcmd -S "localhost\SQLEXPRESS01" -d MESManager -E -C -Q "SELECT Stato, COUNT(*) AS Conteggio FROM Commesse GROUP BY Stato" -W -s "|"

Write-Host "`nExpected commesse status in Mago:" -ForegroundColor Yellow
sqlcmd -S "192.168.1.72\SQLEXPRESS01" -d TODESCATO_NET -U Gantt -P Gantt2019 -C -Q "SELECT 'Aperte' AS Stato, COUNT(*) AS Conteggio FROM MA_SaleOrd WHERE Delivered = 0 AND Invoiced = 0 UNION ALL SELECT 'Consegnate', COUNT(*) FROM MA_SaleOrd WHERE Delivered = 1 AND Invoiced = 0 UNION ALL SELECT 'Fatturate', COUNT(*) FROM MA_SaleOrd WHERE Invoiced = 1" -W -s "|"
...
sqlcmd -S "localhost\SQLEXPRESS01" -d MESManager -E -C -Q "SELECT TOP 10 Modulo, CONVERT(VARCHAR(20), DataOra, 120) AS DataOra, Nuovi, Aggiornati, Ignorati, Errori, LEFT(MessaggioErrore, 100) AS Errore FROM LogSync ORDER BY DataOra DESC" -W -s "|"
