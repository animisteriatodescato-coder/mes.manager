    ...
    public static async Task SeedAsync(MesManagerDbContext db, ILogger logger)
    {
        ...
        if (!await db.PriceLists.AnyAsync())
        {
            var priceList = new PriceList
            {
                ...
                Name = "Listino E2E",
                Version = "2026.01",
                Description = "Listino seed per E2E",
                ValidFrom = now.Date,
                IsDefault = true,
                ItemCount = 1,
                CreatedAt = now,
        ...
        {
            var defaultPriceList = await db.PriceLists.FirstAsync();
            var defaultItem = await db.PriceListItems.FirstAsync();
            ...
            var quote = new Quote
            {
                ...
                PriceListVersionSnapshot = defaultPriceList.Version,
                Subtotal = 55m,
                DiscountTotal = 0m,
                TaxableAmount = 55m,
                VatTotal = 12.1m,
            ...
            var row = new QuoteRow
            {
                ...
                PriceListItemId = defaultItem.Id,
                Code = defaultItem.Code,
                Description = defaultItem.Description,
                Quantity = 10m,
                Unit = defaultItem.Unit,
                UnitPrice = defaultItem.BasePrice,
                DiscountPercent = 0m,
                VatPercent = defaultItem.VatRate,
                RowTotal = 55m,
                VatAmount = 12.1m,
                Notes = "Seed E2E"
            };
